...
_REQUEST_DB_PATH: ContextVar[Path | None] = ContextVar("caddie_request_db_path", default=None)
...
def get_db():
    ...
    active_path = _REQUEST_DB_PATH.get() or DB_PATH
    active_path.parent.mkdir(parents=True, exist_ok=True)
    conn = sqlite3.connect(str(active_path), timeout=30)
...
def set_request_db_path(path: Path | None):
    """Route database access for the current request to an isolated workspace."""
...
def init_db():
    ...
    """)
    c.execute("""
        CREATE TABLE IF NOT EXISTS chat_confirmation_states (
            message_id INTEGER NOT NULL REFERENCES chat_messages(id) ON DELETE CASCADE,
            action_key TEXT NOT NULL,
            state TEXT NOT NULL,
            updated_at TEXT DEFAULT (datetime('now','localtime')),
            PRIMARY KEY(message_id, action_key)
        )
    """)
    ...
    """)
    c.execute("""
        CREATE TABLE IF NOT EXISTS followup_answer_versions (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            ...
            created_by TEXT DEFAULT 'user',
            created_at TEXT DEFAULT (datetime('now','localtime')),
            UNIQUE(followup_id, version)
        )
    """)
    c.execute("""
        CREATE TABLE IF NOT EXISTS followup_coaching_versions (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            followup_id INTEGER NOT NULL REFERENCES followups(id) ON DELETE CASCADE,
    ...
    """)
    c.execute("""
        CREATE TABLE IF NOT EXISTS followup_coaching_messages (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            followup_id INTEGER NOT NULL REFERENCES followups(id) ON DELETE CASCADE,
            role TEXT NOT NULL,
...
def set_chat_confirmation_state(message_id, action_key, state):
    if state not in {"applied", "kept", "dismissed"}:
        return False
    conn = get_db()
    exists = conn.execute(
        "SELECT 1 FROM chat_messages WHERE id=? AND role='assistant'", (message_id,)
    ).fetchone()
    if not exists:
        conn.close()
        return False
    conn.execute("""INSERT INTO chat_confirmation_states(message_id,action_key,state)
                    VALUES (?,?,?)
                    ON CONFLICT(message_id,action_key) DO UPDATE SET
                    state=excluded.state,updated_at=datetime('now','localtime')""",
                 (message_id, action_key, state))
    conn.commit(); conn.close()
    return True
...
def get_chat_history(session_id, limit=30):
    ...
    if result:
        ...
        confirmation_rows = rows(conn.execute(
            f"SELECT message_id,action_key,state FROM chat_confirmation_states "
            f"WHERE message_id IN ({placeholders})",
            ids,
        ).fetchall())
        confirmation_grouped = {}
        for row in confirmation_rows:
            confirmation_grouped.setdefault(row["message_id"], {})[row["action_key"]] = row["state"]
        for item in result:
            item["confirmation_states"] = confirmation_grouped.get(item["id"], {})
...
def _decode_followup_coaching(row):
    item = dict(row) if row else None
    if not item:
        return None
    for field in ("strengths", "issues", "satisfaction_criteria", "followups", "evidence_gaps"):
        raw = item.pop(f"{field}_json", "[]")
        try:
            item[field] = json.loads(raw or "[]")
        except (TypeError, json.JSONDecodeError):
            item[field] = []
    item["interviewer_intent"] = item.get("likely_intent") or ""
    item["why_asked"] = item.get("why_relevant") or ""
    return item


def get_followup_workspace(fid):
    conn = get_db()
    followup = one(conn.execute("""SELECT f.*,p.name AS project_name,p.experience_id,
        e.company,e.role FROM followups f
    ...
    if not followup:
        conn.close()
        return None
    versions = rows(conn.execute("""SELECT * FROM followup_answer_versions
        WHERE followup_id=? ORDER BY version DESC,id DESC""", (fid,)).fetchall())
    coaching = _decode_followup_coaching(conn.execute("""SELECT * FROM followup_coaching_versions
        WHERE followup_id=? ORDER BY version DESC,id DESC LIMIT 1""", (fid,)).fetchone())
    messages = rows(conn.execute("""SELECT * FROM followup_coaching_messages
        WHERE followup_id=? ORDER BY id""", (fid,)).fetchall())
    for message in messages:
        try:
            message["proposal"] = json.loads(message.pop("proposal_json", "{}") or "{}")
        except (TypeError, json.JSONDecodeError):
            message["proposal"] = {}
    conn.close()
    return {
        "followup": followup,
        "current_answer": versions[0]["body"] if versions else (followup.get("answer") or ""),
...
def save_followup_answer_version(fid, body, created_by="user"):
    conn = get_db()
    version = conn.execute("SELECT COALESCE(MAX(version),0)+1 n FROM followup_answer_versions WHERE followup_id=?", (fid,)).fetchone()["n"]
    conn.execute("INSERT INTO followup_answer_versions(followup_id,body,version,created_by) VALUES(?,?,?,?)",
                 (fid, body or "", version, created_by))
    conn.execute("UPDATE followups SET answer=?,updated_at=datetime('now','localtime') WHERE id=?", (body or "", fid))
    conn.commit(); conn.close()
    return version


def save_followup_coaching_version(fid, data):
    conn = get_db()
    version = conn.execute("SELECT COALESCE(MAX(version),0)+1 n FROM followup_coaching_versions WHERE followup_id=?", (fid,)).fetchone()["n"]
    conn.execute("""INSERT INTO followup_coaching_versions
        (followup_id,source_answer_version,likely_intent,why_relevant,answer_summary,
         strengths_json,issues_json,satisfaction_criteria_json,answer_strategy,improved_answer,
         followups_json,evidence_gaps_json,confidence_note,instruction,model_provider,model_name,version)
        VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""", (
        fid, data.get("source_answer_version") or 0,
        data.get("interviewer_intent") or data.get("likely_intent"),
        data.get("why_asked") or data.get("why_relevant"), data.get("answer_summary"),
        ...
        data.get("confidence_note"), data.get("instruction"), data.get("model_provider"),
        data.get("model_name"), version,
    ))
    conn.commit(); conn.close()
    return version


def save_followup_coaching_message(fid, data):
    conn = get_db()
    cur = conn.execute("""INSERT INTO followup_coaching_messages
        (followup_id,role,content,proposal_json,coaching_version,answer_version,model_provider,model_name)
        VALUES(?,?,?,?,?,?,?,?)""", (
        fid, data.get("role"), data.get("content") or "",
        json.dumps(data.get("proposal") or {}, ensure_ascii=False),
        data.get("coaching_version") or 0, data.get("answer_version") or 0,
...
def get_followup_coaching_message(mid):
    conn = get_db(); row = one(conn.execute("SELECT * FROM followup_coaching_messages WHERE id=?", (mid,)).fetchone()); conn.close()
    if row:
        try: row["proposal"] = json.loads(row.pop("proposal_json", "{}") or "{}")
        except (TypeError, json.JSONDecodeError): row["proposal"] = {}
    return row


def mark_followup_coaching_message_applied(mid):
    conn = get_db(); conn.execute("UPDATE followup_coaching_messages SET applied=1 WHERE id=?", (mid,)); conn.commit(); conn.close()
