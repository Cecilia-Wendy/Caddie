...
# The hosted beta is a separate application edition. Its local database,
# provider credentials and vault must never share the stable Caddie directory.
os.environ.setdefault("CADDIE_DATA_DIR", str(Path.home() / ".caddie-hosted-alpha"))

PORT = int(os.environ.get("CADDIE_PORT", "8767"))
        ...
        def after_gui_started():
            ...
            try:
                subprocess.run(
                    [
                        "/usr/bin/osascript", "-e",
                        'tell application id "app.caddie.hosted-alpha" to activate',
                    ],
