...
APP_BUNDLE_NAME = "Caddie Hosted Alpha.app"
APP_EXECUTABLE_NAME = "Caddie Hosted Alpha"
APP_BUNDLE_ID = "app.caddie.hosted-alpha"
...
def verify_manifest(manifest: dict) -> dict:
    ...
    if manifest.get("bundle_id") != APP_BUNDLE_ID:
...
def _find_staged_app(root: Path) -> Path:
    candidates = list(root.rglob(APP_BUNDLE_NAME))
    if len(candidates) != 1:
        raise UpdateError(f"更新包中没有唯一的 {APP_BUNDLE_NAME}")
    return candidates[0]
...
def create_apply_helper() -> dict:
    ...
    backup_app = UPDATE_DIR / "previous-Caddie-Hosted-Alpha.app"
