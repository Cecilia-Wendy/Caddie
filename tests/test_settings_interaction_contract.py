...
class SettingsInteractionContractTests(unittest.TestCase):
    ...
    def test_ai_usage_dashboard_is_automatic(self):
        self.assertIn('id="aiUsageDashboard"', HTML)
        self.assertIn("loadAiUsage", HTML)
        self.assertIn("/ai/usage", HTML)
        self.assertNotIn('id="f_monthly_tokens"', HTML)
        self.assertIn("用量由每次模型响应自动统计，不需要填写", HTML)
        self.assertIn(".ai-usage-dashboard", CSS)
