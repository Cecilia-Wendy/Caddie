...
class HostedGatewayTest(unittest.TestCase):
    def setUp(self):
    ...
    def test_hosted_client_has_separate_runtime_identity(self):
        root = Path(__file__).parents[1]
        main = (root / "main.py").read_text(encoding="utf-8")
        launchers = (root / "agent_launchers.py").read_text(encoding="utf-8")
        self.assertIn('Path.home() / ".caddie-hosted-alpha"', main)
        self.assertIn('os.environ.get("CADDIE_PORT", "8767")', main)
        self.assertIn("caddie-hosted-alpha-mcp", launchers)
        self.assertIn("Caddie Hosted Alpha.app", launchers)
