...
_DEMO_SESSION_COOKIE = "caddie_demo_session"
_DEMO_SESSION_RE = re.compile(r"^[a-f0-9]{32}$")
_DEMO_UPLOAD_PATHS = (
    re.compile(r"^/api/chat/attachments$"),
    re.compile(r"^/api/sources/upload$"),
    re.compile(r"^/api/opportunities/import-file$"),
    re.compile(r"^/api/projects/\d+/import$"),
    re.compile(r"^/api/import/resume$"),
    re.compile(r"^/api/companies/.+/logo$"),
    re.compile(r"^/api/company-submission-materials/\d+/upload$"),
    re.compile(r"^/api/v2/interview-growth-analyses/\d+/mock/voice$"),
)


def _public_demo_enabled() -> bool:
    return os.environ.get("CADDIE_PUBLIC_DEMO", "").strip().lower() in {
        "1", "true", "yes", "on",
    }


def _demo_template_db() -> Path:
    raw = os.environ.get("CADDIE_DEMO_TEMPLATE_DB", "").strip()
    return Path(raw) if raw else db.CADDIE_DATA_DIR / "demo-template" / "caddie.db"


def _demo_session_root() -> Path:
    raw = os.environ.get("CADDIE_DEMO_SESSION_ROOT", "").strip()
    return Path(raw) if raw else db.CADDIE_DATA_DIR / "demo-sessions"


def _demo_session_db(session_id: str) -> Path:
    return _demo_session_root() / session_id / "caddie.db"


def _cleanup_demo_sessions() -> None:
    root = _demo_session_root()
    if not root.exists():
        return
    ttl_hours = max(1, int(os.environ.get("CADDIE_DEMO_SESSION_TTL_HOURS", "6")))
    max_sessions = max(5, int(os.environ.get("CADDIE_DEMO_MAX_SESSIONS", "80")))
    ...
    for index, item in enumerate(items):
        if index >= max_sessions or item.stat().st_mtime < cutoff:
            shutil.rmtree(item, ignore_errors=True)


def _ensure_demo_session(session_id: str) -> Path:
    template = _demo_template_db()
    if not template.exists():
        raise RuntimeError(f"Demo template database is missing: {template}")
    session_db = _demo_session_db(session_id)
    if not session_db.exists():
        _cleanup_demo_sessions()
        session_db.parent.mkdir(parents=True, exist_ok=True)
        shutil.copy2(template, session_db)
    os.utime(session_db.parent, None)
    return session_db


def _demo_upload_blocked(path: str, method: str) -> bool:
    return method in {"POST", "PUT", "PATCH"} and any(
        pattern.match(path) for pattern in _DEMO_UPLOAD_PATHS
    )


def _start_request_context_thread(target, *, args=(), name: str):
    """Keep an isolated Demo database bound inside durable request workers."""
    context = copy_context()
    worker = threading.Thread(
        target=lambda: context.run(target, *args),
        name=name,
        daemon=True,
    )
    worker.start()
...
def public_demo_status():
    ...
    return {
        ...
        "session_ttl_hours": max(1, int(os.environ.get("CADDIE_DEMO_SESSION_TTL_HOURS", "6"))),
        "ai_enabled": bool(ai.get_active_provider()),
        "notice": "当前为虚构数据体验环境，请勿输入任何真实个人或公司信息。",
    }


@app.post("/api/demo/reset")
def reset_public_demo_workspace():
    if not _public_demo_enabled():
        raise HTTPException(404, "Demo 模式未开启")
    active_db = db.current_db_path()
    root = _demo_session_root().resolve()
    try:
        active_db.resolve().relative_to(root)
    except ValueError as exc:
        raise HTTPException(409, "当前请求未进入隔离的 Demo 空间") from exc
    template = _demo_template_db()
    replacement = active_db.with_suffix(".resetting")
    shutil.copy2(template, replacement)
    for suffix in ("-wal", "-shm"):
        Path(str(active_db) + suffix).unlink(missing_ok=True)
    os.replace(replacement, active_db)
    return {"ok": True, "message": "已恢复为初始虚构求职数据"}
...
async def local_request_guard(request, call_next):
    ...
    if _public_demo_enabled():
        if _demo_upload_blocked(path, request.method):
            return Response(
                "公开 Demo 已禁用文件上传，请勿提交真实简历或隐私资料。",
                status_code=403,
            )
        candidate = (request.cookies.get(_DEMO_SESSION_COOKIE) or "").lower()
        demo_session_id = candidate if _DEMO_SESSION_RE.fullmatch(candidate) else uuid.uuid4().hex
        try:
            demo_session_db = _ensure_demo_session(demo_session_id)
        except OSError as exc:
            return Response(f"Demo 体验空间初始化失败：{exc}", status_code=503)
        demo_db_token = db.set_request_db_path(demo_session_db)
    ...
    try:
        ...
        if demo_db_token is not None:
            db.reset_request_db_path(demo_db_token)
    if feature_action:
        feature, action = feature_action
        telemetry.log_event(
            "feature_action_completed",
    ...
    if demo_session_id:
        response.set_cookie(
            ...
            demo_session_id,
            max_age=max(3600, int(os.environ.get("CADDIE_DEMO_SESSION_TTL_HOURS", "6")) * 3600),
            httponly=True,
            secure=True,
            samesite="none",
        )
...
class ProviderIn(StrictInputModel):
    ...
    monthly_token_limit: Optional[int] = Field(None, ge=0, le=10_000_000_000)


class HostedActivateIn(StrictInputModel):
...
def _schedule_interview_decomposition(task_id: int, rid: int, source_id: int,
    ...
    worker = _start_request_context_thread(
        _run_interview_decomposition_task,
...
def _schedule_interview_question_coach(
    ...
    with _INTERVIEW_WORKERS_LOCK:
        if task_id in _INTERVIEW_WORKERS:
            return False
        _INTERVIEW_WORKERS.add(task_id)
    _start_request_context_thread(
        _run_interview_question_coach_task,
...
def _schedule_interview_growth(task_id: int, analysis_id: int, mock_count: int = 10):
    with _INTERVIEW_WORKERS_LOCK:
        ...
        _INTERVIEW_WORKERS.add(task_id)
    _start_request_context_thread(
        _run_interview_growth_task,
        args=(task_id, analysis_id, mock_count),
        name=f"interview-growth-{task_id}",
    )
...
class AgentMigrationStartIn(BaseModel):
    ...
    recipe: Literal["full_workspace", "resume", "experience_project", "interview"] = "full_workspace"
...
@app.get("/api/hosted/status")
def hosted_status():
    return hosted_access.quota()


@app.post("/api/hosted/activate")
def hosted_activate(body: HostedActivateIn):
    try:
        result = hosted_access.activate(body.invite_code)
        ai.install_hosted_provider()
        return {"ok": True, **result, **hosted_access.quota()}
    except hosted_access.HostedAccessError as exc:
        raise HTTPException(400, str(exc)) from exc
...
def ai_usage(include_remote: bool = True):
    return ai.usage_summary(include_remote=include_remote)
...
@app.post("/api/chat/confirmations/state")
def save_chat_confirmation_state(body: dict):
    try:
        message_id = int((body or {}).get("message_id"))
    except (TypeError, ValueError):
        raise HTTPException(400, "确认状态缺少有效消息")
    action_key = str((body or {}).get("action_key") or "").strip()[:240]
    state = str((body or {}).get("state") or "").strip()
    if not action_key or state not in {"applied", "kept", "dismissed"}:
        raise HTTPException(400, "确认状态无效")
    if not db.set_chat_confirmation_state(message_id, action_key, state):
        raise HTTPException(404, "对应的确认消息不存在")
    return {"ok": True, "message_id": message_id, "action_key": action_key, "state": state}
...
def _agent_integration_paths(home: Path | None = None) -> dict:
    ...
    launcher_suffix = ".cmd" if os.name == "nt" else ""
    installed_mcp = root / ".caddie" / "bin" / f"caddie-mcp{launcher_suffix}"
    installed_cli = root / ".caddie" / "bin" / f"caddie-agent{launcher_suffix}"
    if os.name == "nt":
...
def _migration_task_prompt(task_id: int, body: AgentMigrationStartIn) -> str:
    ...
    recipes = {
        "full_workspace": ("完整求职资料迁移", "盘点全部授权资料，建立个人资料、简历、经历、项目、证据、岗位和面试资料的归档计划"),
        "resume": ("导入简历及其事实", "从简历中拆出个人资料、经历、项目和简历版本，不把整份简历当成一篇普通草稿"),
        "experience_project": ("整理一段经历与项目证据", "建立经历外壳，按真实项目拆分核心事实文档，并将原始材料作为证据关联"),
        "interview": ("归档岗位面试资料", "先定位正确岗位，将自我介绍、业务题、项目话术和复盘分别放入岗位准备区"),
    }
    recipe_name, recipe_goal = recipes[body.recipe]
...
归档路由（必须遵守）
- 姓名、联系方式、教育、目标方向 -> 个人资料，使用 propose_user_profile。
- 一段公司/组织下的实习或工作 -> 我的经历，使用 propose_experience；不得把多个公司合并成一段。
- 经历中可独立讲述的业务问题 -> 该经历下的项目；项目文档只写背景、目标、本人动作、决策逻辑、结果和事实边界。
- 需求文档、周报、数据表、复盘原文 -> 项目的事实证据，保留原路径和引用片段；不得改写后冒充原始证据。
- DOCX/PDF 简历 -> 简历版本，保留文件路径、适配岗位和版本关系；不得放入普通草稿资产。
- JD、投递记录、面试时间 -> 对应岗位记录；未能唯一匹配岗位时先询问用户。
- 自我介绍、业务题、项目话术 -> 对应岗位的“面试准备”，使用 save_career_document(destination="job.interview")。
- 真实面试逐字稿、面试官反馈、结果复盘 -> 对应岗位的“面试复盘”，使用 save_career_document(destination="job.review")；可复用的纠正另提交 propose_feedback。
- 无法准确归档或当前没有对应工具 -> 标记“待处理”，不得为了完成数量放入 draft 或错误岗位。

执行协议
1. 先调用 workspace_bootstrap 和 get_workspace_map，理解 Caddie 已有结构与写入边界。
2. 第一阶段只盘点，不写入。必须以表格输出：来源文件 | 识别类型 | 目标对象 | Caddie 位置 | 准备调用的工具 | 重复/冲突 | 需用户确认。
3. 向我展示迁移方案和“待确认关联”。没有得到我同意前，不创建草稿或候选修改。
...
def start_agent_migration(body: AgentMigrationStartIn):
    ...
    prompt = _migration_task_prompt(task_id, AgentMigrationStartIn(
        agent_key=agent_key, recipe=body.recipe, roots=roots, include_types=body.include_types, notes=body.notes,
    ))
...
def _external_confirmation_receipt(task_id: int, change_id: int | None = None,
                                   pending_count: int = 1) -> dict:
    review_url = f"http://127.0.0.1:8766/?review_task={task_id}"
    if change_id:
    ...
    return {
        ...
        "next_action": f"点击链接审核并确认 {pending_count} 项候选更新。",
        "message_to_user": f"结果已送入 Caddie，还有 {pending_count} 项需确认：{review_url}",
    }
...
def complete_external_agent_run(task_id: int, summary: str = ""):
    ...
    result = _agent_task_payload(db.get_agent_task(task_id))
    result["requires_confirmation"] = bool(pending)
    if pending:
        result["confirmation"] = _external_confirmation_receipt(task_id, pending_count=len(pending))
    return result
...
def propose_external_fact(body: ExternalFactProposalIn):
    ...
    return {"task_id": body.task_id, "change_id": change_id, "requires_confirmation": True,
            "confirmation": _external_confirmation_receipt(body.task_id, change_id)}
...
def apply_agent_task(task_id: int, body: AgentTaskApplyIn):
    ...
    supported = {
        ...
        "create_project", "create_resume_version", "link_project_evidence",
    }
    for change in selected:
        ...
        if action == "create_followup":
            ...
            metadata = _json_loads_safe(change.get("metadata_json"), {}) or {}
            if not db.get_experience(metadata.get("experience_id") or change.get("parent_id")):
                raise HTTPException(400, "项目候选对应的经历已经不存在")
            if not str(metadata.get("name") or "").strip():
                raise HTTPException(400, "项目候选缺少名称")
        elif action == "create_resume_version":
            metadata = _json_loads_safe(change.get("metadata_json"), {}) or {}
            if not db.get_job_track(metadata.get("track_id") or change.get("parent_id")):
                raise HTTPException(400, "简历版本对应的岗位已经不存在")
            if not (metadata.get("docx_path") or metadata.get("pdf_path")):
                raise HTTPException(400, "简历版本缺少本地文件")
        elif action == "link_project_evidence":
            metadata = _json_loads_safe(change.get("metadata_json"), {}) or {}
            if not db.get_project(metadata.get("project_id") or change.get("parent_id")):
                raise HTTPException(400, "证据对应的项目已经不存在")
            if not metadata.get("paths"):
                raise HTTPException(400, "证据候选没有本地文件")
    ...
    applied_labels = []
    for change in selected:
        if change.get("action_type") == "create_project":
            metadata = _json_loads_safe(change.get("metadata_json"), {}) or {}
            experience_id = metadata.get("experience_id") or change.get("parent_id")
            project_id = db.create_project(experience_id, {
                "name": str(metadata.get("name") or "").strip(),
                "one_liner": metadata.get("one_liner") or "",
                "document": metadata.get("document") or "",
                "technologies": metadata.get("technologies") or "",
                "keywords": metadata.get("keywords") or "",
            })
            db.update_proposed_change(change["id"], "applied", target_id=project_id)
            _workspace_event("project_confirmed", "project", project_id, "确认新增项目",
                             "project", project_id, actor_type="user", payload={"task_id": task_id})
            results.append({"change_id": change["id"], "experience_id": experience_id, "project_id": project_id})
            applied_labels.append("项目")
            continue

        if change.get("action_type") == "create_resume_version":
            metadata = _json_loads_safe(change.get("metadata_json"), {}) or {}
            track_id = metadata.get("track_id") or change.get("parent_id")
            payload = _resume_version_payload(ResumeVersionIn(
                version_name=metadata.get("version_name") or change.get("proposed_title") or "简历版本",
                docx_path=metadata.get("docx_path"), pdf_path=metadata.get("pdf_path"),
                status=metadata.get("status") or "editing",
                change_summary=metadata.get("change_summary"),
            ), track_id)
            resume_id = db.create_resume_version(payload)
            db.update_proposed_change(change["id"], "applied", target_id=resume_id)
            _workspace_event("resume_version_confirmed", "resume_version", resume_id,
                             "确认登记简历版本", "track", track_id,
                             actor_type="user", payload={"task_id": task_id})
            results.append({"change_id": change["id"], "track_id": track_id, "resume_version_id": resume_id})
            applied_labels.append("简历版本")
            continue

        if change.get("action_type") == "link_project_evidence":
            metadata = _json_loads_safe(change.get("metadata_json"), {}) or {}
            project_id = metadata.get("project_id") or change.get("parent_id")
            link_result = link_project_local_sources(project_id, ProjectLocalSourcesIn(paths=metadata.get("paths") or []))
            if not link_result.get("linked"):
                raise HTTPException(400, "没有可成功关联的项目证据")
            db.update_proposed_change(change["id"], "applied", target_id=project_id)
            results.append({"change_id": change["id"], "project_id": project_id, **link_result})
            applied_labels.append("项目证据")
            continue
...
def _followup_coaching_document(coaching: dict | None) -> str:
    item = coaching or {}
    def lines(values):
        return "\n".join(f"- {value}" for value in (values or [])) or "无"
    return f"""可能考察点：{item.get('interviewer_intent') or '待分析'}
为什么与项目相关：{item.get('why_asked') or '待分析'}
当前答案问题：
{lines(item.get('issues'))}
满意标准：
{lines(item.get('satisfaction_criteria'))}
...
def _generate_followup_coaching(fol: dict, source_answer: str, instruction: str = ""):
    ctx = caddie_context.build_context(
        project_id=fol.get("project_id"), intent="followup_answer"
    )
    material = "\n\n".join(part for part in (
        f"【项目模拟追问】\n{fol.get('question') or ''}",
        f"【用户当前答案】\n{source_answer or '暂无'}",
    ...
    system = """你是严谨的项目经历面试教练。当前题目是用户为项目准备的模拟追问，
不是已经发生的真实面试题。你要形成可长期保存的深度解析。

边界：
1. 只能说“可能考察”“通常会关注”，不得声称某位面试官当时的真实意图。
2. 诊断必须基于当前答案和项目材料；不得编造职责、数字、结果或协作关系。
3. 区分表达问题、逻辑问题、事实缺口和岗位风险。
4. 事实不足时写入 EVIDENCE_GAPS，并在参考答法中使用“[待补充：...]”。
5. 参考答法必须可口述，且只重组已有事实。

严格使用以下纯文本分隔符，不要返回 JSON：
===INTENT===
这道模拟追问可能考察的核心能力或判断
===WHY_ASKED===
为什么它与当前项目经历相关
...
- 需要用户补充确认的事实
===CONFIDENCE_NOTE===
哪些来自项目材料，哪些只是一般面试规律推断"""
    raw = ai.chat(
        [{"role": "user", "content": material}], system=system,
        max_tokens=3600, timeout=180, profile_key="deep_reasoning",
    )
    parsed = _parse_interview_question_coaching(raw)
    if not _valid_interview_question_coaching(parsed, bool(source_answer.strip())):
        raise RuntimeError("模型输出缺少必要解析章节")
    call_info = ai.get_last_call_info() or {}
    return parsed, {
        "model_provider": call_info.get("provider_name") or call_info.get("provider_id"),
        "model_name": call_info.get("model"),
...
@app.get("/api/followups/{fid}/workspace")
def get_followup_workspace(fid: int):
    workspace = db.get_followup_workspace(fid)
    if not workspace:
        raise HTTPException(404, "追问不存在")
    return {"ok": True, "data": workspace}


@app.put("/api/followups/{fid}/answer")
def save_followup_answer(fid: int, body: dict):
    fol = db.get_followup(fid)
    if not fol:
        raise HTTPException(404, "追问不存在")
    answer = str(body.get("body") or "").strip()
    version = db.save_followup_answer_version(fid, answer, body.get("created_by") or "user")
    _commit("保存项目追问答案版本")
    return {"ok": True, "data": {"version": version, "workspace": db.get_followup_workspace(fid)}}


@app.post("/api/followups/{fid}/coach")
def coach_followup(fid: int, body: dict):
    workspace = db.get_followup_workspace(fid)
    if not workspace:
        raise HTTPException(404, "追问不存在")
    try:
        coaching, model = _generate_followup_coaching(
            workspace["followup"], workspace.get("current_answer") or "",
            str(body.get("instruction") or "").strip(),
        )
    except Exception as exc:
        raise HTTPException(502, f"深度解析失败：{exc}") from exc
    version = db.save_followup_coaching_version(fid, {
        **coaching,
        "source_answer_version": workspace.get("current_version") or 0,
        "instruction": body.get("instruction"), **model,
    })
    _commit("生成项目追问深度解析")
    return {"ok": True, "data": {"version": version, "workspace": db.get_followup_workspace(fid)}}


@app.post("/api/followups/{fid}/coach/messages")
def discuss_followup_coaching(fid: int, body: dict):
    workspace = db.get_followup_workspace(fid)
    if not workspace:
        raise HTTPException(404, "追问不存在")
    message = str(body.get("message") or "").strip()
    if not message:
        raise HTTPException(400, "请输入想追问或纠正的内容")
    coaching = workspace.get("question_coaching")
    if not coaching:
        raise HTTPException(409, "请先完成一次深度解析")
    db.save_followup_coaching_message(fid, {
    ...
    ctx = caddie_context.build_context(
        project_id=workspace["followup"].get("project_id"), intent="followup_answer"
    )
    material = "\n\n".join((
        f"【项目模拟追问】\n{workspace['followup'].get('question')}",
        f"【当前答案】\n{workspace.get('current_answer') or '暂无'}",
        f"【当前深度解析】\n{_followup_coaching_document(coaching)}",
        f"【项目事实】\n{ctx.get('text','')[:10000]}",
        f"【此前对话】\n{history_text}" if history_text else "",
        f"【用户本轮消息】\n{message}",
    ))
    system = """你是与用户共同打磨项目模拟追问的面试教练。先回应用户的异议或追问，
再判断是否需要修改深度解析。用户对自身经历的事实纠正优先级最高，但不要盲目迎合。
不得把模拟题说成真实面试，不得编造事实。只有确需更新时才填写 UPDATE；否则写“无”。
严格按以下分隔符返回：
===REPLY===
直接回应用户并解释依据
===UPDATE_INTENT===
...
- 更新后的证据缺口；无需修改写“无”"""
    try:
        raw = ai.chat([{"role": "user", "content": material}], system=system,
                      max_tokens=2800, profile_key="deep_reasoning")
    except Exception as exc:
        raise HTTPException(502, f"教练对话失败：{exc}") from exc
    parsed = _parse_question_coach_discussion(raw)
    if len(parsed.get("reply") or "") < 10:
        raise HTTPException(502, "教练没有返回可用解释")
    call_info = ai.get_last_call_info() or {}
    mid = db.save_followup_coaching_message(fid, {
        "role": "assistant", "content": parsed["reply"], "proposal": parsed["proposal"],
        "coaching_version": coaching.get("version") or 0,
        "answer_version": workspace.get("current_version") or 0,
        "model_provider": call_info.get("provider_name") or call_info.get("provider_id"),
        "model_name": call_info.get("model"),
    ...
    _commit("记录项目追问教练对话")
    return {"ok": True, "data": {"message_id": mid, "workspace": db.get_followup_workspace(fid)}}


@app.post("/api/followups/{fid}/coach/messages/{message_id}/apply")
def apply_followup_coaching_message(fid: int, message_id: int):
    workspace = db.get_followup_workspace(fid)
    message = db.get_followup_coaching_message(message_id)
    if not workspace or not message or message.get("followup_id") != fid:
        raise HTTPException(404, "候选修改不存在")
    proposal = message.get("proposal") or {}
    if message.get("role") != "assistant" or not proposal:
        raise HTTPException(400, "本轮对话没有候选修改")
    current = workspace.get("question_coaching") or {}
    merged = {key: current.get(key) for key in (
        "interviewer_intent", "why_asked", "answer_summary", "strengths", "issues",
        "satisfaction_criteria", "answer_strategy", "improved_answer", "followups",
        "evidence_gaps", "confidence_note",
    )}
    merged.update(proposal)
    version = db.save_followup_coaching_version(fid, {
        **merged, "source_answer_version": workspace.get("current_version") or 0,
        "instruction": f"采纳教练对话消息 #{message_id}",
        "model_provider": message.get("model_provider"), "model_name": message.get("model_name"),
    })
    db.mark_followup_coaching_message_applied(message_id)
    _commit("采纳项目追问教练建议")
    return {"ok": True, "data": {"version": version, "workspace": db.get_followup_workspace(fid)}}
